...
st.title("🛢️ Oil Options Delta-Gamma Hedging w/ Trade Price")
st.markdown("**Trade Price → MTM → P&L** | Buy=-cost | Sell=+income | Futures MTM=0")
...
r = st.sidebar.number_input("Risk-Free (%)", value=5.0)/100
T_days = st.sidebar.slider("Days to Expiry", 1, 90, 30)
T = T_days/365

# Portfolio w/ TRADE PRICE
...
for i in range(5):
    with st.expander(f"Position {i+1}", expanded=(i==0)):
        ...
        if position_type == "futures":
            col5.info("🛢️ Δ=±1.0 | MTM=0 at spot")
        elif bbls != 0:
        ...
        if bbls != 0:
            if position_type == "futures":
                ...
                current_price = 0.0  # Futures MTM = 0 at spot
            else:
                greeks = black_scholes_greeks(S, K, T, r, IV, position_type)
                current_price = greeks['price']
            
            # TRADE VALUE (what you paid/received)
            trade_value = bbls * trade_price
            
            # CURRENT MTM VALUE (mark-to-market)
...
# **PERFECT POSITION TABLE**
if position_data:
    st.subheader("📊 Trade Book w/ P&L")
    ...
    col1.metric("**Net Δ (bbl)**", f"{net_delta:+,}")
    col2.metric("**Net Γ (bbl)**", f"{net_gamma:.1f}")
    col3.metric("**Total P&L**", f"${net_pnl:,.0f}")
    ...
    if st.button("🎯 Run Shock Simulation", use_container_width=True):
        ...
        for p in positions:
            ...
            shock_pnl_data.append({
                'Pos': f"{p['bbls']:+,} {p['type'].upper()}",
                'Shock P&L': f"${shock_pnl:,.0f}"
            })
        
        col1, col2 = st.columns(2)
        col1.metric("**Shock P&L**", f"${new_total_pnl:,.0f}", delta_color="inverse")
        col2.metric("**New Net Δ**", f"{new_net_delta:+,} bbl")
        
        st.subheader("💥 Position Shock P&L")
        st.table(shock_pnl_data)
        
        st.info(f"**Rehedge**: {'🎯 NEUTRAL' if abs(new_net_delta)<1 else f"{'BUY' if new_net_delta<0 else 'SELL'} {abs(new_net_delta):,.0f} bbl futures"}")
...
if positions:
    ...
    fig.update_layout(title="Portfolio P&L vs Spot Price", xaxis_title="WTI Spot ($)", yaxis_title="P&L ($)", height=500)
    st.plotly_chart(fig, use_container_width=True)

st.markdown("---")
st.caption("**Real Oil Desk** | Trade Price → MTM → P&L | Perfect delta hedging")
